...
# Load resource group name from deployment configuration
if [ -f "deployment_config.json" ]; then
    RESOURCE_GROUP=$(jq -r '.RESOURCE_GROUP_NAME' deployment_config.json)
else
    echo "❌ deployment_config.json not found. Please run the deployment script first."
    exit 1
fi

# Verify jq is installed
if ! command -v jq &> /dev/null; then
    echo "❌ jq is not installed. Please install jq (brew install jq or apt-get install jq)"
    exit 1
fi
